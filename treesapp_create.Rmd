...
-   Transfer the nwk file in the 'treesapp create'  output directory  `XmoA_seed/final_outputs/XmoA.nwk` to your desktop

```{bash eval = FALSE}
scp  root@<server address>:data/ts_tutorial/XmoA_seed/final_outputs/XmoA.nwk  <path to your local directory>
```

-   Visit the [iTOL](https://itol.embl.de/) website.

-   Upload the file `XmoA.nwk` from the XmoA directory. This file contains both the phylogeny of the placements that will be visualized.

-   Navigate to the page displaying the phylogeny in the iTOL window. Use the NCBI accessions option to convert the TreeSAPP identifiers (e.g. 1_XmoA) to more useful descriptions with the organism name and accession for each leaf node.

-   the figure should look identical to Figure \@ref(fig:xmoa).

![(\#fig:xmoa) Phylogenetic tree of XmoA_seed refpkg.  There were 36 sequences classified that are part of the XmoA seed package ](images/XmoA_36_iTOL.png)
