...
In the [Acquiring reference sequence data] tutorial, PmoA & AmoA sequences were downloaded from [EggNOG](http://eggnog5.embl.de/#/app/home) and [FunGene](http://fungene.cme.msu.edu/). Here, we will use the EggNOG sequences as well as seed sequences from [TIGRFAM](http://tigrfams.jcvi.org/cgi-bin/index.cgi) --- a database that is no longer updated but was well curated. Let's call this our seed reference package. The steps involved are:
...
-   Let's look briefly at the newly generated fasta file using `cat` and `less`. `cat` stands for concatenate, i.e. the files are pasted together. The output of `cat` is funneled with the `>` symbol to a new file, i.e.  `cat file_1.ext file_2.ext > new_file.ext` 

```{bash eval = FALSE}
cat ENOG5028JPK_EggNOGv5.faa TIGR03080.faa arCOG08676_EggNOGv5.faa | less -S
```
...
```{bash eval = FALSE}
cat ENOG5028JPK_EggNOGv5.faa TIGR03080.faa arCOG08676_EggNOGv5.faa | grep -c "^>"
```
...
treesapp create \
  ...
  -m prot \
  -p 0.97 \
  --fastx_input ENOG5028JPK_EggNOGv5.faa TIGR03080.faa arCOG08676_EggNOGv5.faa \
  -c XmoA \
  --output XmoA_seed
```
...
```
 ...
 -  **On your computer:** Transfer `XmoA_seed/XmoA_labelled_tree.txt` from the server to your local directory `Xmoa_sequences`.
...
-   Upload the file `XmoA_labelled_tree.txt`. You can do this using either your computer's file browser then navigating to the parent directory and clicking-and-dragging the file, or iTOL's file uploader---the "Upload tree files" button. 

-   The figure should look identical to this figure below Figure \@ref(fig:xmoa).

    ![(\#fig:xmoa) Phylogenetic tree of the seed XmoA reference package.](images/XmoA_seed_iTOLtree.svg){width=100%}
