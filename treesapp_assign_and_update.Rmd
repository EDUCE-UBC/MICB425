...
- Classify the 6966 protein sequences in `UniProt_XmoA.fasta` using `treesapp assign`.
...
Finally, we are going to require that taxonomic lineages are resolved to at least the rank of Phylum with the argument `--min_taxonomic_rank`.
...
It looks like 107 new sequences were introduced into the reference package, bringing the total to 143 sequences.
...
Just to finish this off, let's map the PmoA and AmoA sequences from UniProt onto the MAG-updated reference package with `treesapp assign` to view later.
...
With the CuMMO feature annotation included in the reference package, we can use TreeSAPP to create iTOL-compatible annotation files for colouring the phylogeny.
...
-   The two files `XmoA_CuMMO_colours_style.txt` and `XmoA_CuMMO_colour_strip.txt` can be dragged into the iTOL browser window to bring colour to the XmoA tree. The figure should look identical to Figure \@ref(fig:XmoA-Classified)
