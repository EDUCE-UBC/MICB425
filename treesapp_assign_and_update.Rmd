...
On your local computer follow the steps for downloading the `iTOL_outputs` directory from the server and uploading the files to iTOL as we did [previously](#the-interactive-tree-of-life).

-   the figure should look identical to Figure \@ref(fig:XmoA-Updated-tree)

![(\#fig:XmoA-Updated-tree)Updated tree topology with added 28 new sequences including Archaea and Bacteria ](images/XmoA_update_ref_tree_iTOL.png)
...
You can view the contents of this file with the Unix command `cat`:
```{bash eval = FALSE}
cat XmoA_taxonomy-phenotype_map.tsv
d__Archaea	AmoA_AOA
...
```{bash eval = FALSE}
scp -r root@10.19.139.201:/data/ts_tutorial/AmoA_assign/iTOL_output <path to Xmoa_sequences>
```
