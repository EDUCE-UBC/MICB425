...
-   **On the server:** Move to your directory `/data` (if it is not your working directory already) and create a new directory called `<user>/ts_tutorial` where `<user>/` is unique within your project group.
...
-   **On your computer:** Open a second terminal window that is connected to your local computer, then copy the reference sequences from your computer to your server.
    ...
    ```{bash eval = FALSE}
    scp <path to ts_tutorial_local>/* root@<server address>:/data/<user>/ts_tutorial/
    ```
