...
Here, we will use the gene names AmoA and PmoA to find reference sequences in UniProt. Begin by navigating to [https://www.uniprot.org/](https://www.uniprot.org/) and typing either "Particulate methane monooxygenase" (including the double quotation marks) into the search bar at the top of the page.
...
To remove these off-target search results, we will need to expand our search terms and for this we will turn to the "Advanced" drop-down menu on the right side of the search bar. Add "pmoa" for the gene name and remove all sequences by specifying "uncultured bacterium" with the "NOT" logical operator. The latter filter was applied since these poorly annotated sequences are typically derived from short environmental samples and are not taxonomically informative.
...
Downloading sequences in UniProt is quick and easy. Click the ![](images/uniprot_dl_icon.png){width=50} button (atop the table), ensure the format is FASTA, and click "Go" to specify where the file should be saved on your computer.
...
To gather all sequences in UniProt for the XmoA reference package, repeat the previous two steps with the string "Ammonia monooxygenase alpha subunit".
